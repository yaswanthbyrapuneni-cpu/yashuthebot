...
interface SecurityMonitorProps {
  ...
  onSecurityStateChange?: (isMonitoring: boolean, isManualOverride: boolean) => void; // Notify parent when monitoring starts/stops
}
...
export default function SecurityMonitor({ 
  ...
  useEffect(() => {
    const checkSecurityMode = async () => {
      try {
        ...
        const isOverride = settings?.manual_override || false;
        
        if (shouldBeActive !== securityModeEnabled || isOverride !== manualOverrideEnabled) {
          console.log('[Security Monitor] Security mode/override changed:', shouldBeActive, isOverride);
          setSecurityModeEnabled(shouldBeActive);
          setManualOverrideEnabled(isOverride);
          
          if (shouldBeActive && !securityModeEnabled) {
            await startMonitoring();
          } else if (!shouldBeActive && securityModeEnabled) {
            stopMonitoring();
          }
          
          // Notify parent
          if (onSecurityStateChange) {
            onSecurityStateChange(shouldBeActive, isOverride);
          }
        }
    ...
    checkSecurityMode();
    const checkInterval = setInterval(checkSecurityMode, 10000); // Check every 10 seconds for faster kiosk responsiveness

    return () => {
      clearInterval(checkInterval);
    };
  }, [kioskId, isActive, securityModeEnabled, manualOverrideEnabled]);
  ...
  const startMonitoring = async () => {
    ...
    // Pre-load face detection model so first detection is instant
    try {
      await loadDetector('face');
      console.log('[Security Monitor] Face detector loaded');
    } catch (err) {
      console.warn('[Security Monitor] Face detector failed to load:', err);
    }
    ...
    monitoringIntervalRef.current = window.setInterval(async () => {
      ...
      try {
        ...
        // Face detection
        const faceResult = runDetection('face', videoRef.current);
        const faceDetected = (faceResult?.faceLandmarks?.length ?? 0) > 0;

        if (faceDetected && !faceStateRef.current.isDetected) {
        ...
        } else if (!faceDetected && faceStateRef.current.isDetected) {
    ...
    <>
      ...
      {securityModeEnabled && (
        <div className="fixed top-4 right-4 bg-red-600 text-white px-4 py-2 rounded-lg shadow-lg z-50">
          ...
          {lastMotionDetected && (
            <div className="text-sm mt-1">⚠️ Motion Detected</div>
          )}
          {lastFaceDetected && (
            <div className="text-sm mt-1">🚨 Face Detected</div>
          )}
        </div>
      )}
      ...
      {cameraError && securityModeEnabled && (
        <div className="fixed top-20 right-4 bg-yellow-600 text-white px-4 py-2 rounded-lg shadow-lg z-50">
          <div className="text-sm">⚠️ Camera Error: {cameraError}</div>
        </div>
      )}
